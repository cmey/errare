...
public class OggInputStream extends FilterInputStream {
		...
		while (!eos && !fetchedPacket) {
			...
			if (result1 == 0) {
				...
				if (result2 == 0) {
					...
					Logger.printINFO("syncState.pageout(page) result == -1");
					return -1;
				} else {
				...
				Logger.printINFO("streamState.packetout(packet) result == -1");
				return -1;
			} else {
				fetchedPacket = true;
			}
	...
	public static void main(String args[]) {
		try {
			...
			Logger.printINFO(byteOut.size() + " bytes read");
			Logger.printINFO(oggInput.toString());
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
