...
public class Matrix{
	...
	// Construct matrix with a reference to the 2D matrix and permutation index of an existing ComplexMatrix bb.
    	...
    	public void setArray(float[] array){
    		int s = array.length;
    		s=(int) Math.sqrt(s);
    		double[][] darr = new double[s][s];
    		for(int i=0;i<s;i++){
    			for(int j=0;j<s;j++){
    				darr[i][j]=(double)array[i*s+j];
    			}
    		}
    		setTwoDarray(darr);
    	}
    		...
    		if(matrix.length>0){
    			...
    			for(int i=0; i<matrix.length;i++)
    				for(int j=0; j<matrix[0].length;j++)
    					res[i*matrix[0].length+j]=(float)matrix[i][j];
    			return res;
    		}else
    			return null;
    	}
    	...
    	public float[] GetRow(int r){
    		float[] res = new float[matrix[r].length];
    		for(int i=0; i<matrix[r].length;i++)
    			res[r]=(float)matrix[r][i];
    		return res;
    	}
   	...
   	// Multiply a matrix by a constant [static method]
	    ...
	    public Matrix luDecomp(){
	    	...
	    	for (int i=0;i<n;i++) {
        		...
        		if (big == 0.0D){
        	    		Logger.printWARNING("Attempted LU Decomposition of a singular matrix in Matrix.luDecomp()");
         	    		Logger.printWARNING("NaN matrix returned and matrixCheck set to false");
