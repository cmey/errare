...
public class Heightmap extends GraphicalRep{
	...
	public int MAP_SIZE = 1000; //unit OpenGL
	public float alpha=0.6f;
	private float CoefElev = 100; //unit OpenGL
	private int decalage;
	private int list_terrain;
	private int list_terrain_patched;
	public int nb_div;
	...
	private boolean must_recompile_partial;
	private int isx,iex,isy,iey; //bounds to update (index of patch)
	public Water water;
	...
	public Heightmap(String filename, int map_size) {
		...
		decalage=0;
		unpack_texture();
		load_texture("data/images/floor/grass1.png"); //2eme texture (ecrasant la 1ere)
		nb_div=16;
	...
	protected void compile_this_partial(GL gl, int list_index, int x, int y){
		...
		for (int i=x*Patch_size;i<=(x+1)*Patch_size;i++) {
			...
			for (int j=y*Patch_size;j<=(y+1)*Patch_size;j++) {
				...
				if((i+1)<inter.length && j<inter[0].length){
				drawVertex(gl, i+1,j);
				drawVertex(gl, i,j);
				}
			}
