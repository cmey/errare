...
public class ShadowMap extends GraphicalRep{
	...
	public void draw(GLDrawable gld, float px, float py, float pz, float rx, float ry, float rz) {
		...
		gl.glTexImage2D(GL.GL_TEXTURE_2D, 0, GL.GL_LUMINANCE8, shadowMapSize,shadowMapSize,
				0, GL.GL_LUMINANCE, GL.GL_UNSIGNED_BYTE, shadowMapData8Bit);
	...
	public void readShadowMap(GL gl, GLU glu){
		...
		if(prem) {
			...
			gl.glBindTexture(GL.GL_TEXTURE_2D, internal_texture1[0]);
			gl.glTexImage2D(GL.GL_TEXTURE_2D, 0, GL.GL_ALPHA8, shadowMapSize,shadowMapSize,
					0, GL.GL_ALPHA, GL.GL_UNSIGNED_BYTE, shadowMapData8Bit);
			...
			if(!gl.glIsTexture(internal_texture2[0]))
				gl.glGenTextures(1, internal_texture2);
			gl.glBindTexture(GL.GL_TEXTURE_1D, internal_texture2[0]);
			gl.glTexImage1D(GL.GL_TEXTURE_1D, 0, GL.GL_ALPHA8, 256,
					0, GL.GL_ALPHA, GL.GL_UNSIGNED_BYTE,depthRampData8Bit);
		...
		gl.glReadPixels(0, 0, shadowMapSize, shadowMapSize,
				GL.GL_DEPTH_COMPONENT, GL.GL_UNSIGNED_BYTE, shadowMapData8Bit);
